...
data Point = Point { x::Int, y::Int, steps::Int }
  deriving (Show, Eq)
...
manhattanDistance (Point x1 y1 _) (Point x2 y2 _) =
  abs (x1 - x2) + abs (y1 - y2)
...
addTurnToPoint (Point x y d) = \case
  TurnRight i -> Point (x+i) y (d+i)
  TurnLeft i -> Point (x-i) y (d+i)
  TurnUp i -> Point x (y+i) (d+i)
  TurnDown i -> Point x (y-i) (d+i)
